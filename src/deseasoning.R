...
deseasonalise <- \(.data, var_y) {
  ...
  data <- .data %>%
    mutate(weekday = weekdays(date),
           ...
           # Different effects for pre-card donations
           banking = ifelse(date < "2022-05-26" & (monday == 1 | saturday == 1 | sunday == 1), 1, 0))

  formula <- as.formula(paste0(var_y, "~ monday + tuesday + wednesday + thursday + friday + saturday + banking + banking:(monday + saturday)"))
